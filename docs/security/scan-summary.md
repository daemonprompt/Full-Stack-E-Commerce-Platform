...
|---------|----------|---------------------|---------------------------|
| Webhook test endpoint | Medium | Endpoint accepts arbitrary URLs; SSRF probe returned internal response | The full read path: internal service endpoint to session JWT extraction |
...
| Health endpoint verbosity | Low | Health endpoint returns key configuration metadata | That the key prefix and length reduce the brute-force space |
...
SSRF in one service combined with missing auth in the other produces session token access.
...
- Static analysis output: all tools returned exit code 0 on issue files
