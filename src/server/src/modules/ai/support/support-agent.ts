...
/**
 * flow 14: LLM-as-gatekeeper antipattern.
 ...
 *
 * flow 17: Indirect prompt input via seller-controlled product description.
 ...
 *
 * flow 21: IDOR via agentic tool call -- no ownership predicate.
  ...
  while (true) {
    ...
    for (const toolCall of choice.message.tool_calls) {
      ...
      switch (toolCall.function.name) {
        case "issueRefund":
          // flow 14: No cap enforced here. Business logic delegated to system prompt.
          result = await issueRefundTool(args);
          break;

        case "lookupProduct":
          // flow 17: product.description enters LLM context verbatim.
          // Seller-controlled content. No prompt boundary.
          result = await lookupProductTool(args);
          break;

        case "lookupOrder":
          // flow 21: No ownership predicate. customerId not passed to tool.
